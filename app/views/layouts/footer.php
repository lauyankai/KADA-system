<footer class="modern-footer py-3">
    <div class="container">
        <div class="row g-4 mb-3">
            <div class="col-lg-3 col-md-6">
                <h5 class="footer-heading">Pautan Pantas</h5>
                <ul class="footer-links">
                    <li><a href="#"><i class="bi bi-chevron-right"></i> Tentang Kami</a></li>
                    <li><a href="#"><i class="bi bi-chevron-right"></i> Perkhidmatan</a></li>
                    <li><a href="#"><i class="bi bi-chevron-right"></i> Keahlian</a></li>
                    <li><a href="#"><i class="bi bi-chevron-right"></i> Hubungi Kami</a></li>
                    <li><a href="#"><i class="bi bi-chevron-right"></i> FAQ</a></li>
                </ul>
            </div>

            <div class="col-lg-3 col-md-6">
                <h5 class="footer-heading">Perkhidmatan</h5>
                <ul class="footer-links">
                    <li><a href="#"><i class="bi bi-chevron-right"></i> Pinjaman</a></li>
                    <li><a href="#"><i class="bi bi-chevron-right"></i> Pelaburan</a></li>
                    <li><a href="#"><i class="bi bi-chevron-right"></i> Tabung</a></li>
                    <li><a href="#"><i class="bi bi-chevron-right"></i> Pembiayaan</a></li>
                </ul>
            </div>

            <div class="col-lg-6 col-md-12">
                <h5 class="footer-heading">Hubungi Kami</h5>
                <div class="contact-info">
                    <div class="contact-item">
                        <i class="bi bi-geo-alt contact-icon"></i>
                        <span>D/A Lembaga Kemajuan Pertanian Kemubu, P/S 127, 15710 Kota Bharu, Kelantan.</span>
                    </div>
                    <a href="tel:097447088" class="contact-item">
                        <i class="bi bi-telephone contact-icon"></i>
                        <span>09-7447088 samb. 5339 @ 5312</span>
                    </a>
                    <a href="mailto:koperasi_kada@yahoo.com" class="contact-item">
                        <i class="bi bi-envelope contact-icon"></i>
                        <span>koperasi_kada@yahoo.com</span>
                    </a>
                    <div class="contact-item">
                        <i class="bi bi-clock contact-icon"></i>
                        <span>Ahad - Khamis: 8:00 AM - 4:00 PM</span>
                    </div>
                </div>
            </div>
        </div>

        <hr class="footer-divider">

        <div class="row align-items-center footer-bottom g-2">
            <div class="col-md-6">
                <p class="copyright mb-md-0">© <?php echo date('Y'); ?> Koperasi KADA. Hak Cipta Terpelihara. Dibangunkan oleh <a href="https://github.com/lauyankai/KADA-system" class="text-reset">Pelajar UTM</a></p>
            </div>
            <div class="col-md-6">
                <div class="d-flex flex-wrap justify-content-md-end align-items-center gap-3">
                    <div class="social-links">
                        <a href="#" class="social-link"><i class="bi bi-facebook"></i></a>
                        <a href="#" class="social-link"><i class="bi bi-twitter"></i></a>
                        <a href="#" class="social-link"><i class="bi bi-instagram"></i></a>
                        <a href="#" class="social-link"><i class="bi bi-linkedin"></i></a>
                    </div>
                    <div class="legal-links">
                        <a href="#">Dasar Privasi</a>
                        <span class="separator">•</span>
                        <a href="#">Terma</a>
                        <span class="separator">•</span>
                        <a href="#">Penafian</a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</footer>

<!-- Bootstrap JS and other scripts -->
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>